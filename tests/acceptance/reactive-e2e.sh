...
check "走统一 backend::ask_text_only（不硬编码 claude）"  "grep -q 'backend::ask_text_only' src-tauri/src/clipboard_action.rs && ! grep -qE 'find_binary\\(\"claude\"\\)' src-tauri/src/clipboard_action.rs"

echo "── AI 任务串行队列 ──"
check "ai_queue.rs 存在"                                 "test -f src-tauri/src/ai_queue.rs"
check "ai_queue::acquire 串行锁入口"                      "grep -q 'pub async fn acquire' src-tauri/src/ai_queue.rs"
check "reactive action 走 ai_queue"                       "grep -q 'ai_queue::acquire' src-tauri/src/clipboard_action.rs"
check "主 pipeline 走 ai_queue"                           "grep -q 'ai_queue::acquire' src-tauri/src/pipeline.rs"
check "听写 voice_ime 不套队列（保持即时）"               "! grep -q 'ai_queue::acquire' src-tauri/src/voice_ime.rs"
check "忙碌计数 TaskGuard"                                "grep -q 'pub struct TaskGuard' src-tauri/src/reactive.rs"
check "bg-task-changed 事件"                              "grep -q 'EV_BG_TASK_CHANGED' src-tauri/src/reactive.rs"
check "前端监听 bg-task-changed"                          "grep -q 'bg-task-changed' src/App.tsx"
check "前端忙碌 badge"                                    "grep -q 'pet-busy-badge' src/App.tsx"
check "视图离开 idle 清 reactive（通知必达）"             "grep -q 'view.kind !== \"idle\" && reactive' src/App.tsx"

echo "── 多后端兼容 ──"
check "backend::ask_text_only 统一入口"                   "grep -q 'pub async fn ask_text_only' src-tauri/src/backend.rs"
check "ClaudeCli 路径"                                    "grep -q 'Backend::ClaudeCli =>' src-tauri/src/backend.rs"
check "CodexCli 路径"                                     "grep -q 'Backend::CodexCli =>' src-tauri/src/backend.rs"
check "OpenclawCli 路径"                                  "grep -q 'Backend::OpenclawCli =>' src-tauri/src/backend.rs"
check "HermesAgent 路径"                                  "grep -q 'Backend::HermesAgent =>' src-tauri/src/backend.rs"
check "process_reactive_action 错误带 backend 名"          "grep -qE 'backend.display_name\\(\\)' src-tauri/src/clipboard_action.rs"
