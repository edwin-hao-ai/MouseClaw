...
check "Onboarding onComplete 带 voiceImeTrigger"   "grep -q 'voiceImeTrigger: VoiceImeTrigger' src/components/Onboarding.tsx"
check "Onboarding 有 BACKENDS 选项数组"              "grep -q 'const BACKENDS' src/components/Onboarding.tsx"
check "onComplete 调用传 4 参"     "grep -q 'onComplete(selected, backend, skin, voiceImeTrigger)' src/components/Onboarding.tsx"
...
check "Onboarding 是 5 步 (1|2|3|4|5)"            "grep -q 'useState<1 | 2 | 3 | 4 | 5>' src/components/Onboarding.tsx"
check "Onboarding 引入 SKINS"                    "grep -q 'import { SKINS' src/components/Onboarding.tsx"
check "onComplete 4 参完整签名"                   "grep -q 'voiceImeTrigger: VoiceImeTrigger' src/components/Onboarding.tsx"
...
check "ImeTrigger enum 6 项"  "grep -E '    (Fn|Option|Control|RightShift|RightCommand|RightOption),' src-tauri/src/voice_ime.rs | wc -l | tr -d ' ' | grep -q '^6$'"
...
echo "── v0.1.12 · 触发键可选 + ⌘⇧V + Hub 右键 ──"
check "ImeTrigger 6 种选择"                        "grep -q 'pub enum ImeTrigger' src-tauri/src/voice_ime.rs"
check "voice_ime_trigger 在 config"                "grep -q 'voice_ime_trigger' src-tauri/src/config.rs"
check "save_voice_ime_trigger 命令"                "grep -q 'save_voice_ime_trigger' src-tauri/src/lib.rs"
check "托盘有 vime-trigger-submenu"                "grep -q 'vime-trigger-submenu' src-tauri/src/tray.rs"
...
echo "── v0.1.13 · 剪贴板暂停 + Hub history 展开 + 前台监控 + 双击托盘 ──"
check "clipboard PAUSED AtomicBool"                "grep -q 'pub static PAUSED' src-tauri/src/clipboard.rs"
check "clipboard_paused 在 config"                  "grep -q 'clipboard_paused' src-tauri/src/config.rs"
check "save_clipboard_paused 命令注册"              "grep -q 'save_clipboard_paused' src-tauri/src/lib.rs"
check "托盘有 toggle-clipboard-pause"               "grep -q 'toggle-clipboard-pause' src-tauri/src/tray.rs"
check "Hub 显示 expandedSessionId"                  "grep -q 'expandedSessionId' src/components/Hub.tsx"
check "Hub history-turns CSS"                       "grep -q 'hub-history-turns' src/components/Hub.css"
check "voice_ime 前台 bundle 检测"                  "grep -q 'frontmost_bundle' src-tauri/src/voice_ime.rs"
check "tray 双击 → open-hub"                        "grep -q 'DoubleClick' src-tauri/src/tray.rs"

echo "── v0.1.14 · 剩余 prototype P3 全做完 ──"
check "Hub draggable + onDragStart"                 "grep -q 'onDragStart' src/components/Hub.tsx"
check "Hub hover preview 500ms"                     "grep -q 'previewTimerRef' src/components/Hub.tsx"
check "Hub preview CSS"                              "grep -q 'hub-preview' src/components/Hub.css"
check "MouseState 加 type/hub/paste"   "grep -q paste src/components/PixelMouse.tsx && grep -q \"| \\\"type\\\"\" src/components/PixelMouse.tsx"
check "Eyes 处理 type 态"   "grep -E \"state === .type.\" src/components/PixelMouse.tsx"
check "Eyes 处理 hub 态"    "grep -E \"state === .hub.\" src/components/PixelMouse.tsx"
check "Extras 处理 paste 态" "grep -E \"state === .paste.\" src/components/PixelMouse.tsx"
check "ViewKind 加 VoiceImeListening"               "grep -q 'VoiceImeListening' src-tauri/src/events.rs"
check "voice_ime 用 VoiceImeListening 状态"          "grep -q 'VoiceImeListening' src-tauri/src/voice_ime.rs"
check "clipboard_crypto 模块"                        "test -f src-tauri/src/clipboard_crypto.rs"
check "AES-256-GCM 加密"                            "grep -q 'Aes256Gcm' src-tauri/src/clipboard_crypto.rs"
check "Keychain set_generic_password"               "grep -q 'set_generic_password' src-tauri/src/clipboard_crypto.rs"
...
echo "── v0.1.21 · 实时画板 + 工作区 ──"
check "DrawOverlay.tsx 存在"                       "test -f src/DrawOverlay.tsx"
check "main.tsx 路由 draw"                          "grep -F draw src/main.tsx"
check "capabilities 加 draw 窗口"                   "grep -q '"draw"' src-tauri/capabilities/default.json"
check "cursor_trail emit trail-point"              "grep -F emit_to src-tauri/src/cursor_trail.rs && grep -F trail-point src-tauri/src/cursor_trail.rs"
check "show_draw_overlay 创建透明窗口"             "grep -q 'fn show_draw_overlay' src-tauri/src/cursor_trail.rs"
check "set_ignore_cursor_events(true) 点击穿透"    "grep -q 'set_ignore_cursor_events(true)' src-tauri/src/cursor_trail.rs"
check "workspace_path 在 config"                   "grep -q 'workspace_path' src-tauri/src/config.rs"
check "save_workspace_path 命令"                   "grep -q 'pub fn save_workspace_path' src-tauri/src/commands.rs"
check "claude_cli current_dir(workspace)"          "grep -q 'cmd.current_dir' src-tauri/src/claude_cli.rs"
check "backend.rs current_dir(workspace)"          "grep -q 'cmd.current_dir' src-tauri/src/backend.rs"
check "托盘 set-workspace 入口"                    "grep -q 'set-workspace' src-tauri/src/tray.rs"
check "prompt 含工作区路径段"                       "grep -q '当前工作区' src-tauri/src/claude_cli.rs"
