...
pub struct AppState {
    ...
    /// - hold（按住快捷键 / 全局快捷键 PRESS）= true：圈定式，气泡缓动跟随鼠标。
    /// - PetMenu 召唤 = false：原地（anchor）持续监听，气泡稳定 → 头顶文字看得见、「⌨️打字」点得中。
    /// - 托盘召唤 = true（保持原行为：快速 tap）。
    /// emit_view 进 Listening 时按它 + Listening 共同决定开不开 cursor_follow。默认 true。
                ...
                .with_handler(move |app, shortcut, event| {
                    ...
                    match event.state() {
                        ShortcutState::Pressed => {
                            ...
                            // v0.5.x · hold（按住说话）= 圈定式，listening 缓动跟随鼠标。
