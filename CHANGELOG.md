...
### Fixed
- **听写 partial 气泡稳定下来**（用户实测三连修）：
  - **闪烁**：去掉 `.rec-partial` 的 1.6s 透明度脉冲动画 —— 录音红点已示意"在听"，
    文字不必再呼吸式忽明忽暗。
  - **横向抖动**：partial 独占一行 + `max-width`（不再随每个字增长宽度）—— 多行后
    最长行撑满、宽度自然稳定，不再每字 resize 横跳。
  - **横向截断回归**：中途一版误用固定 `width:360`，当 overlay 窗口比它窄时气泡被
    窗口边裁掉 → 改回 `max-width`，气泡随窗口收缩、文字回流换行，永不裁切。
  纵向仅在真换行时缓慢长高（非高频抖动）；长句完整显示不截断。
