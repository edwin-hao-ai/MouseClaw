...
- **本地小模型兜底（Qwen3-0.6B ONNX）**：没装任何 CLI 后端时，基础文本任务（清理/
  ...
  - **清理 / 翻译优先本地（省 token）**：选词菜单的清理/翻译在本地模型就绪时优先走本地（免费、快、离线），失败回退 CLI；解释/写回信仍走 CLI。下载模型本身即 opt-in。
  - **听写"整理"层（类 Typeless，opt-in）**：设置→语音→"听写后整理"开关（默认**关**）。开了之后说完用本地模型把口述清理 + 理条理 + 模糊纠正再写入（~3-5s）。打开开关时后台预热模型避免首用等 ~20s。
  - 性能实测：CPU 冷启动 ~20s（模型加载，每次启动首用一次）/ 预热后 ~3-5s。CoreML EP 实测对动态形状 LLM 更慢，已弃用。
