...
## 沟通：始终用中文回复（HARD RULE，用户语 2026-05-25）

**给用户的所有聊天回复一律用中文（简体）。** 不管用户这条消息用什么语言、不管
任务多技术，回复正文都用中文。只有这些保持英文：代码、文件路径、命令、标识符
（函数名 / crate 名 / `md://`）、commit message、git/PR 正文、以及代码注释里
已有的英文。技术名词（SenseVoice / zipformer / hotwords）可中英夹用，但**句子主体
是中文**。

理由：用户的母语是中文，每次会话都成立的 cross-cutting 偏好，写进规则免得每会话
重申。违反 = 回复不合格。
