...
describe("deriveCompanionFrame", () => {
  it("sleep 优先 —— 键鼠都超 idle 阈值即入睡，覆盖一切", () => {
    const f = deriveCompanionFrame(
      { x: 105, y: 100, sinceKey: IDLE_TO_SLEEP_SECS + 1, sinceMouse: IDLE_TO_SLEEP_SECS + 1 },
      PET_CENTER, WIN,
    );
  ...
  it("没有 petCenter（DOM 还没挂）→ 中性帧", () => {
    const f = deriveCompanionFrame(
      { x: 0, y: 0, sinceKey: 0.1, sinceMouse: 0.1 },
      null, WIN,
    );
  ...
  it("typing pulse 内 → typing 状态（但仍带眼球偏移）", () => {
    const f = deriveCompanionFrame(
      { x: 500, y: 500, sinceKey: TYPING_PULSE_SECS / 2, sinceMouse: 5 },
      PET_CENTER, WIN,
    );
  ...
  it("光标距离 < EXCITED_PX → excited", () => {
    const f = deriveCompanionFrame(
      { x: PET_CENTER.x + EXCITED_PX - 5, y: PET_CENTER.y, sinceKey: 1, sinceMouse: 0 },
      PET_CENTER, WIN,
    );
  ...
  it("EXCITED_PX ≤ 距离 < ALERT_PX → alert", () => {
    const f = deriveCompanionFrame(
      { x: PET_CENTER.x + EXCITED_PX + 10, y: PET_CENTER.y, sinceKey: 1, sinceMouse: 0 },
      PET_CENTER, WIN,
    );
  ...
  it("距离 ≥ ALERT_PX → idle", () => {
    const f = deriveCompanionFrame(
      { x: PET_CENTER.x + ALERT_PX + 10, y: PET_CENTER.y, sinceKey: 1, sinceMouse: 0 },
      PET_CENTER, WIN,
    );
    expect(f.state).toBe("idle");
  });
  ...
  it("eyeOffset 上限不超过 MAX_EYE_OFFSET（远距离 + 远距离 = 饱和）", () => {
    const f = deriveCompanionFrame(
      { x: 10000, y: PET_CENTER.y, sinceKey: 1, sinceMouse: 0 },
      PET_CENTER, WIN,
    );
  ...
  it("光标在桌宠正左 → eyeOffsetX 为负", () => {
    const f = deriveCompanionFrame(
      { x: -500, y: PET_CENTER.y, sinceKey: 1, sinceMouse: 0 },
      PET_CENTER, WIN,
    );
  ...
  it("windowScreenXY 偏移生效（多屏 / 窗口移过）", () => {
    ...
    const f = deriveCompanionFrame(
      { x: 305, y: 400, sinceKey: 1, sinceMouse: 0 },
  ...
  it("typing + sleep 边界 —— 用户刚开始打字（sinceKey 微小）必须叫醒", () => {
    ...
    const f = deriveCompanionFrame(
      { x: 200, y: 200, sinceKey: 0.05, sinceMouse: 100 },
      PET_CENTER, WIN,
    );
    expect(f.state).toBe("typing");
  });
  ...
  it("只敲键不动鼠（写代码典型场景）→ 不睡 + typing", () => {
    const f = deriveCompanionFrame(
      { x: 1000, y: 1000, sinceKey: 0.1, sinceMouse: 60 },
      PET_CENTER, WIN,
    );
    expect(f.state).toBe("typing");
  });
